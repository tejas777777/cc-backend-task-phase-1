...
class CheckoutView(APIView):
    """
    Cart -> Order checkout. This is the Phase 2 baseline: correct on the happy path and under
    sequential requests, wrapped in a single DB transaction so our own writes are all-or-nothing.
    It does NOT yet defend against concurrent checkouts racing for the same stock (no row locking)
    or duplicate submissions (no idempotency key) - Phase 3 hardens both.
    """
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsBuyer]
